...
class DownScaleLoss(nn.Module):
    ...
    def __init__(self, scale_factor):
        ...
        self.bicubic_kernel = torch.Tensor([[0.0001373291015625, 0.0004119873046875, -0.0013275146484375, -0.0050811767578125, -0.0050811767578125, -0.0013275146484375, 0.0004119873046875, 0.0001373291015625],
                     ...
                     [0.0004119873046875, 0.0012359619140625, -0.0039825439453125, -0.0152435302734375, -0.0152435302734375, -0.0039825439453125, 0.0012359619140625, 0.0004119873046875],
                     [0.0001373291015625, 0.0004119873046875, -0.0013275146484375, -0.0050811767578125, -0.0050811767578125, -0.0013275146484375, 0.0004119873046875, 0.0001373291015625]]).cuda()
...
def loss_sum_to_one(kernel):
    return F.l1_loss(
        kernel.sum().unsqueeze(0),
        torch.ones(1).to(kernel.device),
        reduction="mean"
    )


def loss_boundaries(kernel):
    mask = create_penalty_mask(kernel.size(0), 30).to(kernel.device)
    return F.l1_loss(
        kernel * mask, 
        torch.zeros_like(kernel * mask).to(kernel.device), 
        reduction="sum"
    )
    

def loss_center(kernel, scale_factor):
    indices = torch.arange(0., float(kernel.size(0))).to(kernel.device)
    wanted_center_of_mass = kernel.size(0) // 2 + 0.5 * (int(1 / scale_factor) - kernel.size(0) % 2)
    center = torch.FloatTensor([wanted_center_of_mass, wanted_center_of_mass]).to(kernel.device)
    r_sum, c_sum = torch.sum(kernel, dim=1).reshape(1, -1), torch.sum(kernel, dim=0).reshape(1, -1)
    input_tensor = torch.stack(
        (
            torch.matmul(r_sum, indices) / torch.sum(kernel),
            torch.matmul(c_sum, indices) / torch.sum(kernel)
        )
    )
    return F.mse_loss(input_tensor[:,0], center, reduction="sum")

def loss_sparse(kernel):
    return F.l1_loss(
        kernel.abs().sqrt(),
